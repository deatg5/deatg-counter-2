window.handlers = {
    ...
    saveCounter(event) {
        ...
        const increaseAmountRaw = document.getElementById('counterIncreaseAmount').value;
        const decreaseAmountRaw = document.getElementById('counterDecreaseAmount').value;

        const updatedCounter = {
            ...editingCounter,
            name: document.getElementById('counterName').value,
            count: Number(document.getElementById('counterCount').value),
            increaseAmount: increaseAmountRaw.trim() === '' ? undefined : Number(increaseAmountRaw),
            decreaseAmount: decreaseAmountRaw.trim() === '' ? undefined : Number(decreaseAmountRaw)
        };
    ...
    saveGlobalSettingsFromInputs() {
        const increaseRaw = document.getElementById('globalIncreaseAmount').value;
        const decreaseRaw = document.getElementById('globalDecreaseAmount').value;
        appState.globalSettings.increaseAmount = increaseRaw.trim() === '' ? 1 : Number(increaseRaw);
        appState.globalSettings.decreaseAmount = decreaseRaw.trim() === '' ? 1 : Number(decreaseRaw);
        appState.globalSettings.richPresenceEnabled = document.getElementById('richPresenceEnabled').checked;
        window.electron.updateGlobalSettings(appState.globalSettings);
    }
};
