...
contextBridge.exposeInMainWorld('electron', {
    ...
    updateGlobalSettings: (settings: Partial<GlobalSettings>) => ipcRenderer.send('update-global-settings', settings),
    ...
    onDiscordStatus: (callback: (status: { enabled: boolean; connected: boolean }) => void) => ipcRenderer.on('discord-status', (_, data) => callback(data)),
...
declare global {
    interface Window {
        electron: {
            ...
            onDiscordStatus: (callback: (status: { enabled: boolean; connected: boolean }) => void) => void;
